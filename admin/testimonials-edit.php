...
if ($_POST && isset($_POST['update_testimonial'])) {
    ...
    $stmt = $pdo->prepare("UPDATE testimonials SET client_name = ?, company = ?, message = ?, rating = ?, image = ?, is_active = ? WHERE id = ?");
    $stmt->execute([$client_name, $company, $message, $rating, $image_path, $is_active, $id]);
    header('Location: testimonials.php');
    exit;
}
?>
        ...
        <div class="form-container">
            <form method="POST" enctype="multipart/form-data">
                <div class="form-group">
                    ...
                    <input type="text" name="client_name" placeholder="Client Name" value="<?php echo htmlspecialchars($testimonial['client_name']); ?>" required>
                </div>
                <div class="form-group">
                    <label>Company</label>
                    <input type="text" name="company" placeholder="Company" value="<?php echo htmlspecialchars($testimonial['company']); ?>" required>
                </div>
                <div class="form-group">
                    <label>Testimonial Message</label>
                    <textarea name="message" placeholder="Testimonial Message" rows="4" required><?php echo htmlspecialchars($testimonial['message']); ?></textarea>
                </div>
                <div class="form-group">
                    <label>Rating</label>
                ...
                <div class="form-group">
                    ...
                    <small style="color: #666;">Upload a new client photo (optional)</small>
                    <?php if ($testimonial['image']): ?>
                        <div id="current-image" style="margin-top: 10px;">
                            <p><strong>Current Image:</strong></p>
                            <img src="../assets/images/<?php echo htmlspecialchars($testimonial['image']); ?>" alt="Current Image" style="max-width: 150px; max-height: 150px; border: 1px solid #ddd; border-radius: 50%;">
                        </div>
                    <?php endif; ?>
                ...
                <div class="form-group">
                    <label><input type="checkbox" name="is_active" <?php echo $testimonial['is_active'] ? 'checked' : ''; ?>> Active</label>
                </div>
